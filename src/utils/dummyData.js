export const data = [
  {
    id: 1,
    slug: "red-label-johnnie-walker",
    name: "Red Label Whisky by Johnnie Walker",
    image: {
      mobile: "/assets/whiskys/black-label.jpg",
      tablet: "/assets/whiskys/black-label.jpg",
      desktop: "/assets/whiskys/black-label.jpg",
    },
    category: "whiskys",
    categoryImage: {
      mobile:
        "/assets/whiskys/black-label.jpg",
      tablet:
        "/assets/whiskys/black-label.jpg",
      desktop:
        "/assets/whiskys/black-label.jpg",
    },
    new: true,
    price: 599,
    description:
      "Descubre el sabor vibrante y el carácter versátil del Red Label de Johnnie Walker, el whisky escocés más vendido del mundo. Perfecto para cócteles o disfrutarlo solo, este blend combina notas de especias, frutas frescas y un toque ahumado que lo hace ideal para cualquier ocasión.",
    features:
      "El Red Label de Johnnie Walker es un whisky icónico reconocido por su calidad y consistencia. Ideal para mezclas, este blend combina más de 35 maltas y granos seleccionados. Disfruta de su perfil de sabor característico: especias cálidas, frutas dulces y un final ahumado sutil.\n\nDisponible en presentaciones de 750 ml y 1 litro, este whisky escocés es perfecto para celebraciones, reuniones con amigos o simplemente para relajarte después de un largo día. Su versatilidad lo hace una opción popular en todo el mundo, desde cocteles clásicos como un Johnnie & Ginger hasta nuevas creaciones para cada ocasión.",
    seo: {
      keywords: [
        "Red Label Johnnie Walker",
        "Whisky escocés",
        "Whisky premium",
        "Mejores whiskies",
        "Compra whisky online",
        "Whisky cerca de mí",
        "Johnnie Walker Red Label precio",
        "Whisky para cócteles"
      ],
      title: "Red Label Johnnie Walker - Whisky escocés Premium",
      metaDescription:
        "Compra el Red Label de Johnnie Walker, el whisky escocés más vendido del mundo. Perfecto para cocteles o disfrutar solo, con notas de especias y frutas frescas.",
      locationTags: [
        "whisky en Municipio",
        "whisky en Toluca",
        "whisky cerca de mí",
        "whisky en Toluca"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Red Label (750 ml)",
      },
      {
        quantity: 1,
        item: "Etiqueta de garantía y autenticidad",
      },
      {
        quantity: 1,
        item: "Folleto informativo sobre la historia de Johnnie Walker",
      },
    ],
    others: [
      {
        slug: "jack-daniels-old-no-7",
        name: "Jack Daniel's Old No. 7",
        image: {
          mobile: "/assets/whiskys/jack-daniels-old-no7.webp",
          tablet: "/assets/whiskys/jack-daniels-old-no7.webp",
          desktop: "/assets/whiskys/jack-daniels-old-no7.webp",
        },
      },
      {
        slug: "buchanans-deluxe-12",
        name: "Buchanan's Deluxe 12",
        image: {
          mobile: "/assets/whiskys/buchanans-12.webp",
          tablet: "/assets/whiskys/buchanans-12.webp",
          desktop: "/assets/whiskys/buchanans-12.webp",
        },
      },
      {
        slug: "chivas-regal-12",
        name: "Chivas Regal 12",
        image: {
          mobile: "/assets/whiskys/chivas-regal.jpg",
          tablet: "/assets/whiskys/chivas-regal.jpg",
          desktop: "/assets/whiskys/chivas-regal.jpg",
        },
      },
    ],
  },
  {
    id: 2,
    slug: "don-julio-70-cristalino",
    name: "Tequila Don Julio 70 Cristalino",
    image: {
      mobile: "/assets/tequilas/donjulio.png",
      tablet: "/assets/tequilas/donjulio.png",
      desktop: "/assets/tequilas/donjulio.png",
    },
    category: "tequilas",
    categoryImage: {
      mobile:
        "/assets/tequilas/donjulio.png",
      tablet:
        "/assets/tequilas/donjulio.png",
      desktop:
        "/assets/tequilas/donjulio.png",
    },
    new: true,
    price: 899,
    description:
      "Descubre el equilibrio perfecto entre la tradición y la innovación con el Tequila Don Julio 70 Cristalino. Este tequila premium ofrece la suavidad de un añejo con la frescura de un blanco, gracias a su triple destilación y filtrado único. Ideal para disfrutar en las mejores ocasiones, ya sea solo, en las rocas o en un cóctel sofisticado.",
    features:
      "Don Julio 70 Cristalino es un tequila excepcionalmente suave y complejo, creado para redefinir la experiencia del tequila premium. Elaborado con 100% agave azul y envejecido en barricas de roble blanco americano, este tequila ofrece un perfil único con notas de vainilla, miel, cítricos y un toque ligero de roble tostado.\n\nDisponible en presentaciones de 750 ml, Don Julio 70 Cristalino es perfecto para cenas elegantes, reuniones con amigos o para regalar en ocasiones especiales. Su presentación sofisticada lo convierte en un verdadero símbolo de lujo en el mundo de los tequilas.",
    seo: {
      keywords: [
        "Tequila Don Julio 70 Cristalino",
        "Tequila premium",
        "Mejores tequilas",
        "Tequila cerca de mí",
        "Don Julio 70 precio",
        "Compra tequila online",
        "Tequila para cócteles",
        "Tequila añejo cristalino"
      ],
      title: "Don Julio 70 Cristalino - Tequila Premium de Tradición",
      metaDescription:
        "Compra Don Julio 70 Cristalino, el tequila premium que combina tradición e innovación. Perfecto para disfrutar solo, en las rocas o en cócteles, con notas suaves de vainilla y miel.",
      locationTags: [
        "tequila en Municipio",
        "tequila en Ciudad de México",
        "tequila en Toluca",
        "tequila cerca de mí",
        "tequila en Estado de México"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Don Julio 70 (750 ml)",
      },
      {
        quantity: 2,
        item: "Etiqueta de garantía y autenticidad",
      },
      {
        quantity: 1,
        item: "Folleto informativo sobre la historia de Don Julio",
      },
    ],
    others: [
      {
        slug: "don-julio-1942",
        name: "Tequila Don Julio 1942",
        image: {
          mobile: "/assets/tequilas/don-julio-1942.webp",
          tablet: "/assets/tequilas/don-julio-1942.webp",
          desktop: "/assets/tequilas/don-julio-1942.webp",
        },
        seo: {
          keywords: [
            "Tequila Don Julio 1942",
            "Tequila premium",
            "Mejores tequilas",
            "Tequila cerca de mí",
            "Don Julio 1942 precio",
            "Compra tequila online",
            "Tequila para cócteles",
            "Tequila añejo 1942",
            "Tequila para regalo",
            "Compra tequila online",
          ],
          title: "Don Julio 1942 - Tequila Premium de Tradición",
          metaDescription:
            "Compra el exclusivo Tequila Don Julio 1942, un añejo premium perfecto para ocasiones especiales. Descubre su sabor único con notas de caramelo, vainilla y roble.",
        }
      },
      {
        slug: "tequila-1800-cristalino",
        name: "Tequila 1800 Cristalino",
        image: {
          mobile: "/assets/tequilas/1800-cristalino.webp",
          tablet: "/assets/tequilas/1800-cristalino.webp",
          desktop: "/assets/tequilas/1800-cristalino.webp",
        },
        seo: {
          keywords: [
            "Tequila 1800 Cristalino",
            "Tequila premium",
            "Mejores tequilas",
            "Tequila cerca de mí",
            "1800 Cristalino precio",
            "Compra tequila online",
            "Tequila para cócteles",
            "Tequila añejo cristalino",
            "Tequila para regalo",
            " Mejores tequilas",
            "Compra tequila cerca de mí",
            "Tequila 1800 Cristalino precio",
            "Tequila 1800 Cristalino online",
            "Tequila 1800 Cristalino para regalo",
          ],
          title: "1800 Cristalino - Tequila Premium de Tradición",
          metaDescription:
            "Descubre el Tequila 1800 Cristalino, un tequila premium con un equilibrio perfecto de suavidad y complejidad. Notas de vainilla, caramelo y roble lo hacen ideal para celebraciones especiales.",
        }
      },
      {
        slug: "jose-cuervo-tradicional-plata",
        name: "Tequila José Cuervo Tradicional Plata",
        image: {
          mobile: "/assets/tequilas/jose-cuervo-tradicional-plata.webp",
          tablet: "/assets/tequilas/jose-cuervo-tradicional-plata.webp",
          desktop: "/assets/tequilas/jose-cuervo-tradicional-plata.webp",
        },
        seo: {
          keywords: [
            "Tequila José Cuervo Tradicional Plata",
            "Tequila premium",
            "Mejores tequilas",
            "Tequila cerca de mí",
            "José Cuervo Tradicional Plata precio",
            "Compra tequila online",
            "Tequila para cócteles",
            "Tequila blanco",
            "Tequila para regalo",
            "Mejores tequilas",
            "Mejores tequilas para margaritas,",
            "Compra tequila cerca de mí",
            "Tequila José Cuervo Tradicional Plata precio",
            "Tequila José Cuervo Tradicional Plata online",
            "Tequila José Cuervo Tradicional Plata para regalo",
          ],
          title: "José Cuervo Tradicional Plata - Tequila Blanco Premium",
          metaDescription:
            "Disfruta del Tequila José Cuervo Tradicional Plata, un tequila suave y fresco con notas cítricas y de agave. Perfecto para margaritas o disfrutarlo en las rocas.",
        }
      },
    ],
  },
  {
    id: 3,
    slug: "tequila-1800-cristalino",
    name: "Tequila 1800 Cristalino",
    image: {
      mobile: "/assets/tequilas/1800-cristalino.webp",
      tablet: "/assets/tequilas/1800-cristalino.webp",
      desktop: "/assets/tequilas/1800-cristalino.webp"
    },
    category: "tequilas",
    categoryImage: {
      mobile: "/assets/tequilas/1800-cristalino.webp",
      tablet: "/assets/tequilas/1800-cristalino.webp",
      desktop: "/assets/tequilas/1800-cristalino.webp"
    },
    new: true,
    price: 899,
    description:
      "Descubre el Tequila 1800 Cristalino, un tequila premium con un equilibrio perfecto de suavidad y complejidad. Elaborado 100% de agave azul, este tequila ofrece un perfil único con notas de vainilla, caramelo y roble, ideal para celebraciones especiales o para regalar.",
    features:
      "El Tequila 1800 Cristalino combina la tradición y la innovación. Filtrado para lograr una claridad impecable, este tequila cristalino revela sabores y aromas sutiles que lo hacen perfecto para disfrutar solo, en las rocas o en cócteles exclusivos.\n\nDisponible en presentación de 750 ml, es un símbolo de lujo y sofisticación.",
    seo: {
      keywords: [
        "Tequila 1800 Cristalino",
        "Tequila premium",
        "Mejores tequilas",
        "Tequila cerca de mí",
        "1800 Cristalino precio",
        "Compra tequila online",
        "Tequila para cócteles",
        "Tequila añejo cristalino",
        "Tequila para regalo"
      ],
      title: "Tequila 1800 Cristalino - El Equilibrio Perfecto",
      metaDescription:
        "Compra Tequila 1800 Cristalino, la combinación ideal de suavidad y complejidad. Perfecto para ocasiones especiales o disfrutarlo solo.",
      locationTags: [
        "tequila en Municipio",
        "tequila en Ciudad de México",
        "tequila en Guadalajara",
        "tequila cerca de mí",
        "tequila en Estado de México"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de 1800 Cristalino (750 ml)"
      },
      {
        quantity: 1,
        item: "Etiqueta de garantía y autenticidad"
      }
    ],
    others: [
      {
        slug: "don-julio-blanco",
        name: "Tequila Don Julio Blanco",
        image: {
          mobile: "/assets/tequilas/don-julio-blanco.webp",
          tablet: "/assets/tequilas/don-julio-blanco.webp",
          desktop: "/assets/tequilas/don-julio-blanco.webp"
        },
        seo: {
          keywords: [
            "Tequila Don Julio Blanco",
            "Tequila premium",
            "Tequila blanco Don Julio",
            "Don Julio Blanco precio",
            "Mejores tequilas blancos",
            "Compra tequila online"
          ],
          title: "Don Julio Blanco - La Esencia del Agave",
          metaDescription:
            "Disfruta del Tequila Don Julio Blanco, 100% agave azul, con notas cítricas y un final limpio. Perfecto para cócteles o disfrutarlo solo."
        }
      },
      {
        slug: "hornitos-plata",
        name: "Tequila Hornitos Plata",
        image: {
          mobile: "/assets/tequilas/hornitos-plata.webp",
          tablet: "/assets/tequilas/hornitos-plata.webp",
          desktop: "/assets/tequilas/hornitos-plata.webp"
        },
        seo: {
          keywords: [
            "Tequila Hornitos Plata",
            "Tequila plata",
            "Tequila Hornitos",
            "Hornitos Plata precio",
            "Compra tequila online",
            "Tequila para cócteles"
          ],
          title: "Hornitos Plata - Tradición y Frescura",
          metaDescription:
            "El Tequila Hornitos Plata es perfecto para disfrutar en cócteles o solo. Elaborado 100% de agave azul, es una opción fresca y versátil."
        }
      },
      {
        slug: "maestro-dobel-diamante",
        name: "Tequila Maestro Dobel Diamante",
        image: {
          mobile: "/assets/tequilas/maestro-dobel-diamante.webp",
          tablet: "/assets/tequilas/maestro-dobel-diamante.webp",
          desktop: "/assets/tequilas/maestro-dobel-diamante.webp"
        },
        seo: {
          keywords: [
            "Tequila Maestro Dobel Diamante",
            "Tequila premium",
            "Tequila cristalino",
            "Maestro Dobel Diamante precio",
            "Compra tequila online",
            "Mejores tequilas cristalinos"
          ],
          title: "Maestro Dobel Diamante - Innovación Cristalina",
          metaDescription:
            "Compra Maestro Dobel Diamante, el primer tequila cristalino del mundo. Suave, elegante y perfecto para ocasiones especiales."
        }
      }
    ]
  },
  {
    id: 2,
    slug: "maestro-dobel-diamante",
    name: "Tequila Maestro Dobel Diamante",
    image: {
      mobile: "/assets/tequilas/maestro-dobel-diamante.webp",
      tablet: "/assets/tequilas/maestro-dobel-diamante.webp",
      desktop: "/assets/tequilas/maestro-dobel-diamante.webp"
    },
    category: "tequilas",
    categoryImage: {
      mobile: "/assets/tequilas/maestro-dobel-diamante.webp",
      tablet: "/assets/tequilas/maestro-dobel-diamante.webp",
      desktop: "/assets/tequilas/maestro-dobel-diamante.webp"
    },
    new: true,
    price: 899,
    description: 
      "Maestro Dobel Diamante redefine la excelencia en tequilas premium. Su perfil cristalino combina la riqueza de un añejo y la suavidad de un blanco, ideal para cualquier celebración o regalo especial.",
    features: 
      "Este tequila es el primer añejo cristalino del mundo, con un sabor extraordinariamente suave y equilibrado. Elaborado con 100% agave azul, Maestro Dobel Diamante tiene notas de vainilla, nuez y cítricos, con un toque sutil de roble.\n\nPerfecto para degustarlo solo o con hielo, su presentación elegante lo hace ideal para ocasiones especiales.",
    seo: {
      keywords: [
        "Tequila Maestro Dobel Diamante",
        "Tequila premium",
        "Mejores tequilas cristalinos",
        "Tequila cerca de mí",
        "Maestro Dobel precio",
        "Compra tequila online",
        "Tequila para regalar",
        "Tequila añejo cristalino",
        "Mejores tequilas",
        "Tequila Maestro Dobel",
        "Tequila exclusivo"
      ],
      title: "Maestro Dobel Diamante - El Primer Añejo Cristalino",
      metaDescription:
        "Compra Maestro Dobel Diamante, el primer añejo cristalino con un sabor suave y equilibrado. Ideal para disfrutar solo o en las rocas, perfecto para regalar.",
      locationTags: [
        "tequila en Municipio",
        "tequila en Ciudad de México",
        "tequila en Guadalajara",
        "tequila cerca de mí",
        "tequila en Jalisco"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Maestro Dobel Diamante (750 ml)"
      },
      {
        quantity: 2,
        item: "Etiqueta de autenticidad"
      },
      {
        quantity: 1,
        item: "Estuche de lujo"
      }
    ],
    others: [
      {
        slug: "maestro-tequilero-blanco",
        name: "Tequila Maestro Tequilero Blanco",
        image: {
          mobile: "/assets/tequilas/maestro-tequilero-blanco.webp",
          tablet: "/assets/tequilas/maestro-tequilero-blanco.webp",
          desktop: "/assets/tequilas/maestro-tequilero-blanco.webp"
        },
        seo: {
          keywords: [
            "Tequila Maestro Tequilero Blanco",
            "Tequila blanco",
            "Mejores tequilas",
            "Tequila premium",
            "Compra tequila blanco",
            "Maestro Tequilero precio",
            "Tequila para margaritas",
            "Tequila 100% agave"
          ],
          title: "Maestro Tequilero Blanco - Tequila Puro y Fresco",
          metaDescription:
            "Experimenta la pureza del Tequila Maestro Tequilero Blanco. Perfecto para cócteles o disfrutar su sabor fresco con notas de agave."
        }
      },
      {
        slug: "campo-azul-reposado",
        name: "Tequila Campo Azul Reposado",
        image: {
          mobile: "/assets/tequilas/campo-azul-reposado.webp",
          tablet: "/assets/tequilas/campo-azul-reposado.webp",
          desktop: "/assets/tequilas/campo-azul-reposado.webp"
        },
        seo: {
          keywords: [
            "Tequila Campo Azul Reposado",
            "Tequila reposado",
            "Tequila suave",
            "Compra tequila online",
            "Campo Azul precio",
            "Tequila premium",
            "Mejores tequilas reposados",
            "Tequila para regalo"
          ],
          title: "Campo Azul Reposado - Suavidad y Sabor Premium",
          metaDescription:
            "Descubre Campo Azul Reposado, un tequila con balance perfecto entre suavidad y robustez, ideal para cualquier ocasión."
        }
      },
      {
        slug: "tequila-1800-anejo",
        name: "Tequila 1800 Añejo",
        image: {
          mobile: "/assets/tequilas/1800-anejo.webp",
          tablet: "/assets/tequilas/1800-anejo.webp",
          desktop: "/assets/tequilas/1800-anejo.webp"
        },
        seo: {
          keywords: [
            "Tequila 1800 Añejo",
            "Tequila premium",
            "Tequila añejo",
            "1800 Añejo precio",
            "Mejores tequilas añejos",
            "Compra tequila cerca de mí",
            "Tequila 1800",
            "Tequila para regalo"
          ],
          title: "1800 Añejo - Elegancia y Tradición en Cada Botella",
          metaDescription:
            "El Tequila 1800 Añejo combina tradición y elegancia con un sabor rico en notas de roble, vainilla y especias. Perfecto para disfrutar solo o en las rocas."
        }
      }
    ]
  },
  {
    id: 5,
    slug: "absolut-vodka",
    name: "Absolut Vodka",
    image: {
      mobile: "/assets/vodkas/absolut-vodka-azul.webp",
      tablet: "/assets/vodkas/absolut-vodka-azul.webp",
      desktop: "/assets/vodkas/absolut-vodka-azul.webp"
    },
    category: "vodkas",
    categoryImage: {
      mobile: "/assets/vodkas/absolut-vodka-azul.webp",
      tablet: "/assets/vodkas/absolut-vodka-azul.webp",
      desktop: "/assets/vodkas/absolut-vodka-azul.webp"
    },
    new: true,
    price: 399,
    description:
      "Absolut Vodka es un vodka premium, reconocido por su pureza y sabor suave. Perfecto para cócteles clásicos como el Cosmopolitan o para disfrutarlo solo con hielo. Elaborado con ingredientes naturales, es un vodka de alta calidad que eleva cualquier ocasión.",
    features:
      "Absolut Vodka se elabora exclusivamente con ingredientes naturales, destacando por su pureza y un ligero toque de grano. Sin azúcares añadidos, ofrece un sabor auténtico y versátil.\n\nDisponible en presentaciones de 750 ml, Absolut Vodka es ideal para fiestas, reuniones o como un regalo sofisticado. Su diseño icónico refleja la calidad que define esta marca de prestigio mundial.",
    seo: {
      keywords: [
        "Absolut Vodka",
        "Vodka premium",
        "Mejores vodkas",
        "Vodka cerca de mí",
        "Absolut precio",
        "Compra vodka online",
        "Vodka para cócteles",
        "Vodka Absolut precio",
        "Vodka Absolut Citrus",
        "Vodka para regalo"
      ],
      title: "Absolut Vodka - Vodka Premium de Alta Calidad",
      metaDescription:
        "Compra Absolut Vodka, el vodka premium perfecto para cócteles o para disfrutar solo. Descubre su pureza y sabor auténtico, ideal para cualquier ocasión.",
      locationTags: [
        "vodka en Ciudad de México",
        "vodka en Guadalajara",
        "vodka en Monterrey",
        "vodka cerca de mí",
        "vodka premium online"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Absolut Vodka (750 ml)"
      },
      {
        quantity: 1,
        item: "Etiqueta de garantía y autenticidad"
      }
    ],
    others: [
      {
        slug: "skyy-raspberry",
        name: "Skyy Vodka Raspberry",
        image: {
          mobile: "/assets/vodkas/skyy-raspberry.webp",
          tablet: "/assets/vodkas/skyy-raspberry.webp",
          desktop: "/assets/vodkas/skyy-raspberry.webp"
        },
        seo: {
          keywords: [
            "Skyy Vodka Raspberry",
            "Vodka saborizado",
            "Vodka frambuesa",
            "Skyy Raspberry precio",
            "Compra vodka online",
            "Vodka para cócteles frutales",
            "Vodka para fiestas"
          ],
          title: "Skyy Vodka Raspberry - Vodka Saborizado Premium",
          metaDescription:
            "Prueba Skyy Vodka Raspberry, un vodka premium con un toque dulce de frambuesa. Ideal para cócteles frutales y ocasiones especiales."
        }
      },
      {
        slug: "smirnoff-tamarindo",
        name: "Smirnoff Tamarindo",
        image: {
          mobile: "/assets/vodkas/smirnoff-tamarindo.webp",
          tablet: "/assets/vodkas/smirnoff-tamarindo.webp",
          desktop: "/assets/vodkas/smirnoff-tamarindo.webp"
        },
        seo: {
          keywords: [
            "Smirnoff Tamarindo",
            "Vodka sabor tamarindo",
            "Smirnoff picante",
            "Smirnoff Tamarindo precio",
            "Compra vodka online",
            "Vodka para fiestas",
            "Mezcal con tamarindo"
          ],
          title: "Smirnoff Tamarindo - Vodka con un Toque Picante",
          metaDescription:
            "Descubre Smirnoff Tamarindo, un vodka con un sabor intenso y picante que combina perfectamente con tus cócteles favoritos. ¡Ideal para sorprender a tus invitados!"
        }
      },
      {
        slug: "absolut-citron",
        name: "Absolut Citrus",
        image: {
          mobile: "/assets/vodkas/absolut-citron.webp",
          tablet: "/assets/vodkas/absolut-citron.webp",
          desktop: "/assets/vodkas/absolut-citron.webp"
        },
        seo: {
          keywords: [
            "Absolut Citrus",
            "Vodka cítrico",
            "Vodka Absolut sabor",
            "Absolut Citrus precio",
            "Compra vodka online",
            "Vodka para cócteles frescos",
            "Vodka cítrico premium"
          ],
          title: "Absolut Citrus - Vodka con un Toque Fresco",
          metaDescription:
            "Refresca tus cócteles con Absolut Citrus, un vodka premium con sabores cítricos intensos. Perfecto para margaritas, mojitos y más."
        }
      }
    ]
  },
  {
    id: 6,
    slug: "tanqueray-vodka",
    name: "Tanqueray Vodka",
    image: {
      mobile: "/assets/vodkas/tanqueray-vodka.webp",
      tablet: "/assets/vodkas/tanqueray-vodka.webp",
      desktop: "/assets/vodkas/tanqueray-vodka.webp"
    },
    category: "vodkas",
    categoryImage: {
      mobile: "/assets/vodkas/tanqueray-vodka.webp",
      tablet: "/assets/vodkas/tanqueray-vodka.webp",
      desktop: "/assets/vodkas/tanqueray-vodka.webp"
    },
    new: true,
    price: 699,
    description: 
      "Descubre la suavidad y pureza de Tanqueray Vodka, una opción premium destilada cuatro veces para un sabor inigualable. Perfecto para cócteles sofisticados o para disfrutarlo solo en las mejores ocasiones.",
    features: 
      "Tanqueray Vodka combina tradición y tecnología moderna para ofrecer una experiencia única. Su pureza y suavidad lo convierten en la elección ideal para mixología, fiestas o reuniones. Disponible en presentaciones de 750 ml, Tanqueray Vodka destaca por su calidad excepcional.",
    seo: {
      keywords: [
        "Tanqueray Vodka",
        "Mejores vodkas",
        "Vodka premium",
        "Vodka cerca de mí",
        "Tanqueray Vodka precio",
        "Compra vodka online",
        "Vodka para cócteles",
        "Vodka para regalo"
      ],
      title: "Tanqueray Vodka - La Elegancia del Vodka Premium",
      metaDescription: 
        "Compra Tanqueray Vodka, un vodka premium con una suavidad excepcional. Perfecto para cócteles o para disfrutarlo solo. Disponible online y cerca de ti.",
      locationTags: [
        "vodka en Ciudad de México",
        "vodka en Estado de México",
        "vodka en Monterrey",
        "vodka cerca de mí",
        "vodka premium"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Tanqueray Vodka (750 ml)"
      }
    ],
    others: [
      {
        slug: "zaverich-vodka",
        name: "Zaverich Vodka",
        image: {
          mobile: "/assets/vodkas/zaverich-vodka.webp",
          tablet: "/assets/vodkas/zaverich-vodka.webp",
          desktop: "/assets/vodkas/zaverich-vodka.webp"
        },
        seo: {
          keywords: [
            "Zaverich Vodka",
            "Vodka económico",
            "Compra vodka",
            "Vodka para fiestas",
            "Vodka Zaverich precio",
            "Mejores vodkas",
            "Vodka cerca de mí"
          ],
          metaDescription: 
            "Prueba Zaverich Vodka, la mejor opción en sabor y precio. Ideal para cualquier ocasión. Encuéntralo cerca de ti o cómpralo online."
        }
      },
      {
        slug: "absolut-raspberri",
        name: "Absolut Raspberry",
        image: {
          mobile: "/assets/vodkas/absolut-raspberri.webp",
          tablet: "/assets/vodkas/absolut-raspberri.webp",
          desktop: "/assets/vodkas/absolut-raspberri.webp"
        },
        seo: {
          keywords: [
            "Absolut Raspberry",
            "Vodka sabor frambuesa",
            "Vodka premium",
            "Absolut precio",
            "Vodka Absolut Raspberry",
            "Vodka para cócteles",
            "Vodka cerca de mí"
          ],
          metaDescription: 
            "Descubre Absolut Raspberry, un vodka premium con el delicioso sabor de la frambuesa. Ideal para cócteles únicos y vibrantes."
        }
      },
      {
        slug: "oso-negro-vodka",
        name: "Oso Negro Vodka",
        image: {
          mobile: "/assets/vodkas/oso-negro-vodka.webp",
          tablet: "/assets/vodkas/oso-negro-vodka.webp",
          desktop: "/assets/vodkas/oso-negro-vodka.webp"
        },
        seo: {
          keywords: [
            "Oso Negro Vodka",
            "Vodka económico",
            "Vodka para fiestas",
            "Vodka Oso Negro precio",
            "Compra vodka online",
            "Vodka cerca de mí"
          ],
          metaDescription: 
            "Oso Negro Vodka ofrece calidad y sabor a un precio increíble. Perfecto para reuniones y cócteles. Encuéntralo cerca de ti o cómpralo online."
        }
      }
    ]
  },
  {
    id: 7,
    slug: "smirnoff-tamarindo",
    name: "Smirnoff Tamarindo",
    image: {
        mobile: "/assets/vodkas/smirnoff-tamarindo.webp",
        tablet: "/assets/vodkas/smirnoff-tamarindo.webp",
        desktop: "/assets/vodkas/smirnoff-tamarindo.webp"
    },
    category: "vodkas",
    categoryImage: {
        mobile: "/assets/vodkas/smirnoff-tamarindo.webp",
        tablet: "/assets/vodkas/smirnoff-tamarindo.webp",
        desktop: "/assets/vodkas/smirnoff-tamarindo.webp"
    },
    new: true,
    price: 239,
    description:
        "Disfruta del sabor picante y dulce del Smirnoff Tamarindo, un vodka infusionado que combina notas de tamarindo con un toque de chile. Perfecto para cocteles o disfrutarlo solo con hielo.",
    features:
        "El Smirnoff Tamarindo es una opción única para quienes buscan sabores intensos y atrevidos. Elaborado con vodka de la más alta calidad y un infusionado especial de tamarindo con un toque picante.\n\nIdeal para fiestas, reuniones o para experimentar con cocteles innovadores como el 'Tamarindo Spritz'. Disponible en presentación de 750 ml.",
    seo: {
        keywords: [
            "Smirnoff Tamarindo",
            "Vodka Smirnoff Tamarindo",
            "Smirnoff Tamarindo precio",
            "Compra Smirnoff online",
            "Vodka sabor tamarindo",
            "Vodka con picante",
            "Vodka Smirnoff cerca de mí"
        ],
        title: "Smirnoff Tamarindo - Vodka con Sabor y Picante",
        metaDescription:
            "Compra Smirnoff Tamarindo, el vodka infusionado con un balance perfecto entre dulce y picante. Ideal para cocteles innovadores o disfrutarlo solo con hielo."
    },
    includes: [
        {
            quantity: 1,
            item: "Botella de Smirnoff Tamarindo (750 ml)"
        }
    ],
    others: [
        {
            slug: "absolut-vodka-azul",
            name: "Absolut Vodka Azul",
            image: {
                mobile: "/assets/vodkas/absolut-vodka-azul.webp",
                tablet: "/assets/vodkas/absolut-vodka-azul.webp",
                desktop: "/assets/vodkas/absolut-vodka-azul.webp"
            },
            seo: {
                keywords: [
                    "Absolut Vodka Azul",
                    "Vodka premium",
                    "Absolut precio",
                    "Compra Absolut online",
                    "Vodka cerca de mí",
                    "Absolut Vodka cocteles",
                    "Mejores vodkas"
                ],
                title: "Absolut Vodka Azul - Vodka Premium de Suecia",
                metaDescription:
                    "Compra Absolut Vodka Azul, un vodka premium suave y puro, ideal para cocteles clásicos y modernos. Perfecto para cualquier ocasión."
            }
        },
        {
            slug: "smirnoff-normal",
            name: "Smirnoff Vodka Original",
            image: {
                mobile: "/assets/vodkas/smirnoff-normal.webp",
                tablet: "/assets/vodkas/smirnoff-normal.webp",
                desktop: "/assets/vodkas/smirnoff-normal.webp"
            },
            seo: {
                keywords: [
                    "Smirnoff Vodka Original",
                    "Vodka Smirnoff precio",
                    "Vodka premium",
                    "Vodka para cocteles",
                    "Compra vodka online",
                    "Vodka cerca de mí",
                    "Mejores vodkas"
                ],
                title: "Smirnoff Vodka Original - Clásico y Versátil",
                metaDescription:
                    "Descubre Smirnoff Vodka Original, un clásico mundial perfecto para cocteles o para disfrutar en las rocas. Calidad y sabor garantizados."
            }
        },
        {
            slug: "skyy-vodka",
            name: "Skyy Vodka",
            image: {
                mobile: "/assets/vodkas/skyy.webp",
                tablet: "/assets/vodkas/skyy.webp",
                desktop: "/assets/vodkas/skyy.webp"
            },
            seo: {
                keywords: [
                    "Skyy Vodka",
                    "Vodka Skyy precio",
                    "Vodka premium",
                    "Vodka americano",
                    "Compra Skyy Vodka online",
                    "Vodka para cocteles",
                    "Vodka cerca de mí"
                ],
                title: "Skyy Vodka - Vodka Premium Americano",
                metaDescription:
                    "Compra Skyy Vodka, un vodka premium americano de sabor suave y fresco. Ideal para cocteles modernos y clásicos."
            }
        }
    ]
  },
  {
    id: 2,
    slug: "buchanans-12-deluxe",
    name: "Buchanan's 12 Años Deluxe",
    image: {
      mobile: "/assets/whiskys/buchanans-12.webp",
      tablet: "/assets/whiskys/buchanans-12.webp",
      desktop: "/assets/whiskys/buchanans-12.webp"
    },
    category: "whiskys",
    categoryImage: {
      mobile: "/assets/whiskys/buchanans-12.webp",
      tablet: "/assets/whiskys/buchanans-12.webp",
      desktop: "/assets/whiskys/buchanans-12.webp"
    },
    new: true,
    price: 799,
    description:
      "Buchanan's 12 Deluxe es un whisky escocés icónico, reconocido por su equilibrio y suavidad. Elaborado con una mezcla de los mejores whiskies de malta y grano, ofrece notas de miel, naranja y madera tostada. Ideal para disfrutar solo, en las rocas o en cócteles elegantes.",
    features:
      "Este whisky de 12 años es envejecido en barricas de roble, lo que le da un sabor rico y distintivo. Perfecto para compartir en ocasiones especiales, Buchanan's 12 Deluxe destaca por su calidad y tradición en el mundo del whisky premium.",
    seo: {
      keywords: [
        "Buchanan's 12 Deluxe",
        "Whisky escocés",
        "Buchanan's precio",
        "Whisky premium",
        "Compra whisky online",
        "Whisky cerca de mí",
        "Mejores whiskys 12 años"
      ],
      title: "Buchanan's 12 Deluxe - Whisky Escocés Premium",
      metaDescription:
        "Compra Buchanan's 12 Deluxe, un whisky escocés premium con 12 años de envejecimiento. Suave, equilibrado y perfecto para ocasiones especiales.",
      locationTags: [
        "whisky en Municipio",
        "whisky en Ciudad de México",
        "whisky en Toluca",
        "whisky cerca de mí",
        "whisky en Estado de México"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Buchanan's 12 Deluxe (750 ml)"
      }
    ],
    others: [
      {
        slug: "jack-daniels-apple",
        name: "Jack Daniel's Apple",
        image: {
          mobile: "/assets/whiskys/jack-daniels-apple.webp",
          tablet: "/assets/whiskys/jack-daniels-apple.webp",
          desktop: "/assets/whiskys/jack-daniels-apple.webp"
        },
        seo: {
          keywords: [
            "Jack Daniel's Apple",
            "Whisky sabor manzana",
            "Whisky premium",
            "Jack Daniel's precio",
            "Whisky para cócteles",
            "Compra whisky online",
            "Mejores whiskys saborizados"
          ],
          title: "Jack Daniel's Apple - Whisky con Sabor a Manzana",
          metaDescription:
            "Descubre Jack Daniel's Apple, una mezcla perfecta de whisky Tennessee con un toque fresco y dulce de manzana verde. Ideal para cócteles o disfrutarlo solo."
        }
      },
      {
        slug: "buchanans-18-special-reserve",
        name: "Buchanan's 18 Años Special Reserve",
        image: {
          mobile: "/assets/whiskys/buchanans-18.webp",
          tablet: "/assets/whiskys/buchanans-18.webp",
          desktop: "/assets/whiskys/buchanans-18.webp"
        },
        seo: {
          keywords: [
            "Buchanan's 18 Special Reserve",
            "Whisky escocés premium",
            "Whisky 18 años",
            "Buchanan's precio",
            "Whisky premium para regalo",
            "Compra whisky online",
            "Whisky de lujo"
          ],
          title: "Buchanan's 18 Special Reserve - Whisky Premium",
          metaDescription:
            "Buchanan's 18 Special Reserve es un whisky escocés de lujo con 18 años de envejecimiento. Suave y elegante, perfecto para celebraciones especiales."
        }
      },
      {
        slug: "johnnie-walker-red-label",
        name: "Johnnie Walker Red Label",
        image: {
          mobile: "/assets/whiskys/jw-red-label.webp",
          tablet: "/assets/whiskys/jw-red-label.webp",
          desktop: "/assets/whiskys/jw-red-label.webp"
        },
        seo: {
          keywords: [
            "Johnnie Walker Red Label",
            "Whisky escocés",
            "Whisky asequible",
            "JW Red Label precio",
            "Whisky para cócteles",
            "Compra whisky online",
            "Mejores whiskys económicos"
          ],
          title: "Johnnie Walker Red Label - Whisky Escocés Versátil",
          metaDescription:
            "Johnnie Walker Red Label es un whisky escocés accesible y versátil, ideal para disfrutar en cócteles o en las rocas. Un clásico en cualquier ocasión."
        }
      }
    ]
  },
  {
    id: 9,
    slug: "buchannans-master",
    name: "Buchanan's Master",
    image: {
      mobile: "/assets/whiskys/buchanans-master.webp",
      tablet: "/assets/whiskys/buchanans-master.webp",
      desktop: "/assets/whiskys/buchanans-master.webp"
    },
    category: "whiskys",
    categoryImage: {
      mobile: "/assets/whiskys/buchanans-master.webp",
      tablet: "/assets/whiskys/buchanans-master.webp",
      desktop: "/assets/whiskys/buchanans-master.webp"
    },
    new: true,
    price: 899,
    description:
      "Buchanan's Master es una mezcla única de whiskies seleccionados a mano por el Master Blender, logrando un perfil de sabor balanceado con notas de frutas maduras, miel y un toque de humo. Ideal para disfrutar solo o con hielo.",
    features:
      "Buchanan's Master es un whisky premium que destaca por su suavidad y profundidad de sabor. Con un equilibrio perfecto entre dulzura y un leve ahumado, este whisky es perfecto para cualquier ocasión especial o para disfrutar en un momento de relajación. Disponible en presentaciones de 750 ml.",
    seo: {
      keywords: [
        "Buchanan's Master",
        "Whisky premium",
        "Whisky Buchanan's",
        "Compra whisky online",
        "Buchanan's Master precio",
        "Mejores whiskies",
        "Whisky cerca de mí"
      ],
      title: "Buchanan's Master - Whisky Premium de Tradición",
      metaDescription:
        "Compra Buchanan's Master, un whisky premium con un sabor balanceado y suave, ideal para celebraciones y momentos especiales. Disponible online y en sucursales."
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Buchanan's Master (750 ml)"
      },
      {
        quantity: 1,
        item: "Etiqueta de autenticidad"
      }
    ],
    others: [
      {
        slug: "jack-daniels-honey",
        name: "Jack Daniel's Honey",
        image: {
          mobile: "/assets/whiskys/jack-daniels-honey.webp",
          tablet: "/assets/whiskys/jack-daniels-honey.webp",
          desktop: "/assets/whiskys/jack-daniels-honey.webp"
        },
        seo: {
          keywords: [
            "Jack Daniel's Honey",
            "Whisky con miel",
            "Jack Daniel's precio",
            "Compra whisky online",
            "Jack Daniel's Honey en oferta",
            "Whisky premium cerca de mí"
          ],
          title: "Jack Daniel's Honey - Whisky con Miel",
          metaDescription:
            "Disfruta del whisky Jack Daniel's Honey, una mezcla suave y dulce con notas de miel y roble. Perfecto para cócteles o para disfrutarlo solo."
        }
      },
      {
        slug: "passport-scotch",
        name: "Passport Scotch",
        image: {
          mobile: "/assets/whiskys/passport-scotch.webp",
          tablet: "/assets/whiskys/passport-scotch.webp",
          desktop: "/assets/whiskys/passport-scotch.webp"
        },
        seo: {
          keywords: [
            "Passport Scotch",
            "Whisky económico",
            "Whisky Passport precio",
            "Compra whisky online",
            "Mejores whiskies baratos",
            "Whisky Passport cerca de mí"
          ],
          title: "Passport Scotch - Whisky Escocés Asequible",
          metaDescription:
            "Passport Scotch, un whisky escocés suave y accesible, perfecto para mezclas y para disfrutar en cualquier ocasión. Descubre su versatilidad."
        }
      },
      {
        slug: "whisky-glenfiddich",
        name: "Whisky Glenfiddich",
        image: {
          mobile: "/assets/whiskys/glenfiddich.webp",
          tablet: "/assets/whiskys/glenfiddich.webp",
          desktop: "/assets/whiskys/glenfiddich.webp"
        },
        seo: {
          keywords: [
            "Whisky Glenfiddich",
            "Whisky escocés",
            "Glenfiddich precio",
            "Compra whisky online",
            "Mejores whiskies escoceses",
            "Whisky Glenfiddich cerca de mí"
          ],
          title: "Glenfiddich - Whisky Escocés Premium",
          metaDescription:
            "Experimenta el whisky Glenfiddich, un escocés premium con notas de frutas, vainilla y roble. Perfecto para conocedores y ocasiones especiales."
        }
      }
    ]
  },
  {
    id: 10,
    slug: "bacardi-blanco-1-75l",
    name: "Bacardí Blanco 1.75L",
    image: {
      mobile: "/assets/rones/bacardi-blanco-1-75l.webp",
      tablet: "/assets/rones/bacardi-blanco-1-75l.webp",
      desktop: "/assets/rones/bacardi-blanco-1-75l.webp"
    },
    category: "ron",
    categoryImage: {
      mobile: "/assets/rones/bacardi-blanco-1-75l.webp",
      tablet: "/assets/rones/bacardi-blanco-1-75l.webp",
      desktop: "/assets/rones/bacardi-blanco-1-75l.webp"
    },
    new: true,
    price: 419,
    description:
      "Descubre el ron Bacardí Blanco en su presentación de 1.75L, ideal para tus mejores cócteles. Con su sabor limpio y suave, este ron es perfecto para mojitos, piñas coladas y otras bebidas refrescantes.",
    features:
      "El Bacardí Blanco es un ron clásico, elaborado con una destilación única para obtener un sabor balanceado. Disfruta de notas sutiles de almendra, vainilla y frutas tropicales, que lo convierten en la base ideal para coctelería o disfrutarlo solo con hielo.",
    seo: {
      keywords: [
        "Bacardí Blanco 1.75L",
        "Ron Bacardí Blanco",
        "Ron para mojitos",
        "Bacardí precio",
        "Compra ron online",
        "Mejores rones blancos",
        "Ron Bacardí cerca de mí"
      ],
      title: "Bacardí Blanco 1.75L - Ron Clásico para Cócteles",
      metaDescription:
        "Compra Bacardí Blanco 1.75L, el ron perfecto para cócteles como mojitos y piñas coladas. Sabor limpio y suave con notas de almendra y frutas tropicales."
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Bacardí Blanco (1.75L)"
      }
    ],
    others: [
      {
        slug: "bacardi-limon-verde",
        name: "Bacardí Limón Verde",
        image: {
          mobile: "/assets/rones/bacardi-limon-verde.webp",
          tablet: "/assets/rones/bacardi-limon-verde.webp",
          desktop: "/assets/rones/bacardi-limon-verde.webp"
        },
        seo: {
          keywords: [
            "Bacardí Limón Verde",
            "Ron Bacardí saborizado",
            "Bacardí Limón precio",
            "Ron sabor limón",
            "Cócteles con Bacardí Limón",
            "Compra Bacardí online",
            "Ron Bacardí cerca de mí"
          ],
          title: "Bacardí Limón Verde - Ron Saborizado para Cócteles",
          metaDescription:
            "Disfruta del Bacardí Limón Verde, un ron saborizado perfecto para cócteles frescos y tropicales. Notas vibrantes de limón para tus mejores mezclas."
        }
      },
      {
        slug: "kraken",
        name: "Kraken Black Spiced Rum",
        image: {
          mobile: "/assets/rones/kraken.webp",
          tablet: "/assets/rones/kraken.webp",
          desktop: "/assets/rones/kraken.webp"
        },
        seo: {
          keywords: [
            "Kraken Black Spiced Rum",
            "Ron oscuro Kraken",
            "Ron especiado premium",
            "Kraken precio",
            "Compra Kraken online",
            "Ron para cócteles oscuros",
            "Ron Kraken cerca de mí"
          ],
          title: "Kraken Black Spiced Rum - Ron Oscuro y Especiado",
          metaDescription:
            "Explora el sabor intenso del Kraken Black Spiced Rum, un ron oscuro con notas de especias y caramelo. Ideal para cócteles audaces o disfrutar solo."
        }
      },
      {
        slug: "bacardi-raspberry",
        name: "Bacardí Raspberry",
        image: {
          mobile: "/assets/rones/bacardi-raspberry.webp",
          tablet: "/assets/rones/bacardi-raspberry.webp",
          desktop: "/assets/rones/bacardi-raspberry.webp"
        },
        seo: {
          keywords: [
            "Bacardí Raspberry",
            "Ron Bacardí sabor frambuesa",
            "Bacardí Raspberry precio",
            "Ron saborizado para cócteles",
            "Cócteles con Bacardí Raspberry",
            "Compra Bacardí online",
            "Ron Bacardí cerca de mí"
          ],
          title: "Bacardí Raspberry - Ron Saborizado con Frambuesa",
          metaDescription:
            "Prueba el Bacardí Raspberry, un ron saborizado con notas frescas de frambuesa. Perfecto para cócteles frutales y refrescantes."
        }
      }
    ]
  },
  {
    id: 11,
    slug: "ron-zacapa-ambar-12-anos",
    name: "Ron Zacapa Ámbar 12 años",
    image: {
      mobile: "/assets/rones/ron-zacapa-ambar-12.webp",
      tablet: "/assets/rones/ron-zacapa-ambar-12.webp",
      desktop: "/assets/rones/ron-zacapa-ambar-12.webp"
    },
    category: "ron",
    categoryImage: {
      mobile: "/assets/rones/ron-zacapa-ambar-12.webp",
      tablet: "/assets/rones/ron-zacapa-ambar-12.webp",
      desktop: "/assets/rones/ron-zacapa-ambar-12.webp"
    },
    new: true,
    price: 799,
    description: 
      "Disfruta del Ron Zacapa Ámbar 12 años, un ron añejado en el corazón de Guatemala, reconocido por su suavidad y complejidad. Ideal para disfrutar solo, en las rocas o en cocteles exclusivos.",
    features:
      "El Ron Zacapa Ámbar 12 años es envejecido mediante un sistema de solera con barricas de roble usadas previamente para whiskys y vinos de Jerez. Ofrece notas de caramelo, vainilla, especias y un toque ahumado.\n\nPerfecto para cenas elegantes, reuniones exclusivas o para regalar a los amantes del ron. Su presentación elegante lo convierte en una elección ideal para cualquier ocasión especial.",
    seo: {
      keywords: [
        "Ron Zacapa Ámbar 12 años",
        "Ron premium",
        "Mejores rones",
        "Ron cerca de mí",
        "Ron Zacapa precio",
        "Compra ron online",
        "Ron para regalo",
        "Ron de Guatemala"
      ],
      title: "Ron Zacapa Ámbar 12 años - Ron Premium Guatemalteco",
      metaDescription:
        "Compra Ron Zacapa Ámbar 12 años, un ron guatemalteco premium con notas de caramelo, vainilla y especias. Perfecto para ocasiones especiales y cocteles sofisticados.",
      locationTags: [
        "ron en Ciudad de México",
        "ron en Guadalajara",
        "ron cerca de mí",
        "ron en Monterrey",
        "ron en Estado de México"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Ron Zacapa Ámbar 12 años (750 ml)"
      }
    ],
    others: [
      {
        slug: "bacardi-carta-blanca",
        name: "Bacardí Carta Blanca",
        image: {
          mobile: "/assets/rones/bacardi-carta-blanca.webp",
          tablet: "/assets/rones/bacardi-carta-blanca.webp",
          desktop: "/assets/rones/bacardi-carta-blanca.webp"
        },
        seo: {
          keywords: [
            "Bacardí Carta Blanca",
            "Ron para cocteles",
            "Ron ligero",
            "Bacardí precio",
            "Mejores rones para mojitos",
            "Compra ron online"
          ],
          title: "Bacardí Carta Blanca - Ron Ligero Ideal para Cócteles",
          metaDescription:
            "Bacardí Carta Blanca es el ron ligero perfecto para mojitos y cocteles frescos. Compra online y descubre su calidad inigualable."
        }
      },
      {
        slug: "kraken-ghost-white",
        name: "Kraken Ghost White",
        image: {
          mobile: "/assets/rones/kraken-white.webp",
          tablet: "/assets/rones/kraken-white.webp",
          desktop: "/assets/rones/kraken-white.webp"
        },
        seo: {
          keywords: [
            "Kraken White",
            "Ron blanco premium",
            "Ron para cocteles",
            "Kraken precio",
            "Compra ron Kraken online",
            "Ron Kraken para mojitos"
          ],
          title: "Kraken White - Ron Blanco Premium",
          metaDescription:
            "Kraken White es un ron blanco premium ideal para cocteles sofisticados. Compra online y disfruta su sabor único."
        }
      },
      {
        slug: "malibu",
        name: "Malibu",
        image: {
          mobile: "/assets/rones/malibu.webp",
          tablet: "/assets/rones/malibu.webp",
          desktop: "/assets/rones/malibu.webp"
        },
        seo: {
          keywords: [
            "Malibu",
            "Ron con coco",
            "Cocteles tropicales",
            "Malibu precio",
            "Compra ron online",
            "Ron para cocteles tropicales"
          ],
          title: "Malibu - Ron con Coco para Cocteles Tropicales",
          metaDescription:
            "Malibu, el ron con coco ideal para cocteles tropicales. Descubre su sabor dulce y suave. Compra online y disfruta en cualquier ocasión."
        }
      }
    ]
  },
  {
    id: 12,
    slug: "ron-bacardi-anejo-750ml",
    name: "Ron Bacardí Añejo 750 ML",
    image: {
      mobile: "/assets/rones/bacardi-anejo.webp",
      tablet: "/assets/rones/bacardi-anejo.webp",
      desktop: "/assets/rones/bacardi-anejo.webp"
    },
    category: "ron",
    categoryImage: {
      mobile: "/assets/rones/bacardi-anejo.webp",
      tablet: "/assets/rones/bacardi-anejo.webp",
      desktop: "/assets/rones/bacardi-anejo.webp"
    },
    new: true,
    price: 549,
    description:
      "Disfruta del Ron Bacardí Añejo 750 ML, un ron premium con un sabor balanceado y notas de caramelo, vainilla y roble. Ideal para cócteles clásicos o para disfrutar solo.",
    features:
      "Ron Bacardí Añejo es elaborado a partir de una mezcla de rones envejecidos hasta por 3 años en barricas de roble. Este ron ofrece una experiencia única con su perfil suave y sofisticado, perfecto para elevar cualquier ocasión especial.\n\nDisponible en presentación de 750 ML, es ideal para preparar cócteles como Cuba Libre o Mojito, o para disfrutarlo solo con hielo.",
    seo: {
      keywords: [
        "Ron Bacardí Añejo 750 ML",
        "Mejores rones",
        "Compra ron premium",
        "Ron Bacardí precio",
        "Ron añejo premium",
        "Ron cerca de mí",
        "Ron Bacardí Añejo para cócteles",
        "Ron Bacardí para regalo"
      ],
      title: "Ron Bacardí Añejo 750 ML - Sabor y Tradición Premium",
      metaDescription:
        "Compra Ron Bacardí Añejo 750 ML, un ron premium con un equilibrio perfecto de caramelo, vainilla y roble. Ideal para cócteles o disfrutarlo solo.",
      locationTags: [
        "ron en Ciudad de México",
        "ron en Guadalajara",
        "ron cerca de mí",
        "ron en Monterrey",
        "ron en Estado de México"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Ron Bacardí Añejo (750 ML)"
      },
      {
        quantity: 1,
        item: "Etiqueta de autenticidad"
      }
    ],
    others: [
      {
        slug: "bacardi-coco",
        name: "Bacardí Coco",
        image: {
          mobile: "/assets/rones/bacardi-coco.webp",
          tablet: "/assets/rones/bacardi-coco.webp",
          desktop: "/assets/rones/bacardi-coco.webp"
        },
        seo: {
          keywords: [
            "Bacardí Coco",
            "Ron sabor coco",
            "Mejores rones tropicales",
            "Bacardí Coco precio",
            "Ron Bacardí sabores",
            "Compra ron tropical",
            "Ron Bacardí Coco cerca de mí"
          ],
          metaDescription:
            "Disfruta del Bacardí Coco, un ron tropical con un sabor refrescante y dulce a coco. Perfecto para cócteles exóticos o disfrutar en la playa."
        }
      },
      {
        slug: "capitan-morgan-spiced-gold",
        name: "Capitán Morgan Spiced Gold",
        image: {
          mobile: "/assets/rones/capitan-morgan.webp",
          tablet: "/assets/rones/capitan-morgan.webp",
          desktop: "/assets/rones/capitan-morgan.webp"
        },
        seo: {
          keywords: [
            "Capitán Morgan Spiced Gold",
            "Ron especiado",
            "Ron para cócteles",
            "Capitán Morgan precio",
            "Ron premium",
            "Compra Capitán Morgan",
            "Capitán Morgan Spiced cerca de mí"
          ],
          metaDescription:
            "Descubre Capitán Morgan Spiced Gold, un ron especiado con notas de vainilla, canela y roble. Perfecto para elevar cualquier cóctel o disfrutarlo solo."
        }
      },
      {
        slug: "bacardi-mango-chile",
        name: "Bacardí Mango Chile",
        image: {
          mobile: "/assets/rones/bacardi-mango-chile.webp",
          tablet: "/assets/rones/bacardi-mango-chile.webp",
          desktop: "/assets/rones/bacardi-mango-chile.webp"
        },
        seo: {
          keywords: [
            "Bacardí Mango Chile",
            "Ron con sabor a mango y chile",
            "Ron Bacardí sabores",
            "Bacardí Mango Chile precio",
            "Ron exótico",
            "Compra ron cerca de mí",
            "Ron Bacardí Mango Chile online"
          ],
          metaDescription:
            "Prueba Bacardí Mango Chile, un ron único con un toque picante y afrutado. Ideal para cócteles atrevidos y momentos especiales."
        }
      }
    ]
  },
  {
    id: 13,
    slug: "brandy-torres-10",
    name: "Brandy Torres 10",
    image: {
      mobile: "/assets/brandies/torres-10.webp",
      tablet: "/assets/brandies/torres-10.webp",
      desktop: "/assets/brandies/torres-10.webp"
    },
    category: "brandies",
    categoryImage: {
      mobile: "/assets/brandies/torres-10.webp",
      tablet: "/assets/brandies/torres-10.webp",
      desktop: "/assets/brandies/torres-10.webp"
    },
    new: true,
    price: 649,
    description:
      "Experimenta la excelencia del Brandy Torres 10, un clásico español con una herencia de calidad y tradición. Envejecido en barricas de roble, ofrece notas de nueces, caramelo y especias, ideal para disfrutar solo o en cócteles exclusivos.",
    features:
      "Torres 10 es un brandy premium elaborado con destilados seleccionados y envejecido con el método tradicional de solera en barricas de roble. Este proceso aporta un carácter único con notas de frutos secos, vainilla y un toque ahumado.\n\nDisponible en presentación de 700 ml, Torres 10 es perfecto para reuniones elegantes, celebraciones especiales o para regalar a los amantes de los destilados de calidad.",
    seo: {
      keywords: [
        "Brandy Torres 10",
        "Brandy premium",
        "Mejores brandies",
        "Brandy español",
        "Torres 10 precio",
        "Compra brandy online",
        "Brandy añejo",
        "Brandy para regalo"
      ],
      title: "Torres 10 - Brandy Español Premium",
      metaDescription:
        "Compra Brandy Torres 10, el clásico español envejecido en barricas de roble. Notas de nueces, caramelo y especias lo hacen perfecto para disfrutar solo o en cócteles."
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Brandy Torres 10 (700 ml)"
      },
      {
        quantity: 1,
        item: "Etiqueta de autenticidad y origen"
      }
    ],
    others: [
      {
        slug: "brandy-presidente-clasico",
        name: "Brandy Presidente Clásico",
        image: {
          mobile: "/assets/brandies/presidente-clasico.webp",
          tablet: "/assets/brandies/presidente-clasico.webp",
          desktop: "/assets/brandies/presidente-clasico.webp"
        },
        seo: {
          keywords: [
            "Brandy Presidente Clásico",
            "Brandy mexicano",
            "Mejores brandies",
            "Brandy Presidente precio",
            "Compra brandy online",
            "Brandy clásico",
            "Brandy para cócteles"
          ],
          metaDescription:
            "Disfruta del Brandy Presidente Clásico, un destilado mexicano con sabor suave y equilibrado. Ideal para cócteles o para disfrutar solo."
        }
      },
      {
        slug: "azteca-de-oro",
        name: "Brandy Azteca de Oro",
        image: {
          mobile: "/assets/brandies/azteca-de-oro.webp",
          tablet: "/assets/brandies/azteca-de-oro.webp",
          desktop: "/assets/brandies/azteca-de-oro.webp"
        },
        seo: {
          keywords: [
            "Brandy Azteca de Oro",
            "Brandy mexicano",
            "Brandy económico",
            "Azteca de Oro precio",
            "Compra brandy online",
            "Mejores brandies",
            "Brandy para cócteles"
          ],
          metaDescription:
            "Prueba el Brandy Azteca de Oro, una opción económica con gran sabor y versatilidad. Ideal para compartir en reuniones o en tus cócteles favoritos."
        }
      },
      {
        slug: "brandy-torres-5",
        name: "Brandy Torres 5",
        image: {
          mobile: "/assets/brandies/torres-5.webp",
          tablet: "/assets/brandies/torres-5.webp",
          desktop: "/assets/brandies/torres-5.webp"
        },
        seo: {
          keywords: [
            "Brandy Torres 5",
            "Brandy español",
            "Brandy añejo",
            "Torres 5 precio",
            "Compra brandy online",
            "Mejores brandies",
            "Brandy para regalo"
          ],
          metaDescription:
            "Descubre el Brandy Torres 5, un destilado español de calidad con notas de vainilla, frutos secos y caramelo. Perfecto para regalar o disfrutar en cualquier ocasión."
        }
      }
    ]
  },
  {
    id: 13,
    slug: "brandy-torres-20",
    name: "Brandy Torres 20",
    image: {
      mobile: "/assets/brandies/brandy-torres-20-700ml.webp",
      tablet: "/assets/brandies/brandy-torres-20-700ml.webp",
      desktop: "/assets/brandies/brandy-torres-20-700ml.webp"
    },
    category: "brandies",
    categoryImage: {
      mobile: "/assets/brandies/brandy-torres-20-700ml.webp",
      tablet: "/assets/brandies/brandy-torres-20-700ml.webp",
      desktop: "/assets/brandies/brandy-torres-20-700ml.webp"
    },
    new: true,
    price: 1350,
    description:
      "Brandy Torres 20 es un brandy de lujo elaborado con soleras exclusivas. Suaves notas de vainilla, frutos secos y cacao lo convierten en una elección ideal para conocedores que buscan lo mejor en destilados.",
    features:
      "Elaborado con uvas Parellada y envejecido en barricas de roble francés, Torres 20 destaca por su suavidad y equilibrio. Perfecto para celebraciones elegantes o momentos especiales. Su botella de diseño sofisticado es también un excelente regalo para amantes del brandy.",
    seo: {
      keywords: [
        "Brandy Torres 20",
        "Brandy de lujo",
        "Brandy premium",
        "Torres 20 precio",
        "Compra brandy online",
        "Mejores brandies",
        "Regalos para conocedores",
        "Brandy cerca de mí"
      ],
      title: "Brandy Torres 20 - Elegancia y Tradición",
      metaDescription:
        "Compra Brandy Torres 20, un destilado premium con suaves notas de vainilla y frutos secos. Ideal para conocedores que buscan lo mejor en brandy."
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Brandy Torres 20 (750 ml)"
      },
      {
        quantity: 1,
        item: "Caja de lujo para regalo"
      }
    ],
    others: [
      {
        slug: "brandy-don-pedro-gran-reserva-especial-750ml",
        name: "Brandy Don Pedro Gran Reserva Especial 750ml",
        image: {
          mobile: "/assets/brandies/don-pedro-gran-reserva.webp",
          tablet: "/assets/brandies/don-pedro-gran-reserva.webp",
          desktop: "/assets/brandies/don-pedro-gran-reserva.webp"
        },
        seo: {
          keywords: [
            "Brandy Don Pedro Gran Reserva Especial",
            "Brandy premium",
            "Mejores brandies",
            "Don Pedro Gran Reserva precio",
            "Brandy para regalo",
            "Compra brandy online",
            "Brandy cerca de mí"
          ],
          title: "Don Pedro Gran Reserva - Brandy Premium",
          metaDescription:
            "Explora el sabor excepcional de Brandy Don Pedro Gran Reserva Especial 750ml. Perfecto para momentos especiales y como regalo elegante."
        }
      },
      {
        slug: "brandy-torres-15",
        name: "Brandy Torres 15",
        image: {
          mobile: "/assets/brandies/brandy-torres-15.webp",
          tablet: "/assets/brandies/brandy-torres-15.webp",
          desktop: "/assets/brandies/brandy-torres-15.webp"
        },
        seo: {
          keywords: [
            "Brandy Torres 15",
            "Brandy premium",
            "Brandy para regalo",
            "Compra brandy online",
            "Mejores brandies",
            "Torres 15 precio",
            "Brandy cerca de mí"
          ],
          title: "Brandy Torres 15 - Suavidad y Elegancia",
          metaDescription:
            "Disfruta del Brandy Torres 15, un destilado premium con notas de caramelo y especias. Ideal para compartir o regalar."
        }
      },
      {
        slug: "hennessy-very-special-cognac",
        name: "Hennessy Very Special Cognac",
        image: {
          mobile: "/assets/brandies/hennessy-vs.webp",
          tablet: "/assets/brandies/hennessy-vs.webp",
          desktop: "/assets/brandies/hennessy-vs.webp"
        },
        seo: {
          keywords: [
            "Hennessy Very Special Cognac",
            "Cognac premium",
            "Hennessy VS precio",
            "Compra cognac online",
            "Mejores cognacs",
            "Cognac para regalo",
            "Cognac cerca de mí"
          ],
          title: "Hennessy Very Special Cognac - Exclusividad y Sabor",
          metaDescription:
            "Compra Hennessy Very Special Cognac, un destilado de lujo con aromas frutales y un toque de especias. Perfecto para ocasiones especiales y regalos exclusivos."
        }
      }
    ]
  },
  {
    id: 14,
    slug: "brandy-torres-15",
    name: "Brandy Torres 15",
    image: {
      mobile: "/assets/brandies/brandy-torres-15.webp",
      tablet: "/assets/brandies/brandy-torres-15.webp",
      desktop: "/assets/brandies/brandy-torres-15.webp"
    },
    category: "brandies",
    categoryImage: {
      mobile: "/assets/brandies/brandy-torres-15.webp",
      tablet: "/assets/brandies/brandy-torres-15.webp",
      desktop: "/assets/brandies/brandy-torres-15.webp"
    },
    new: true,
    price: 799,
    description: 
      "El Brandy Torres 15 es una bebida de lujo con una excelente mezcla de sabores dulces y especiados. Años de maduración en barricas de roble europeo le brindan una suavidad inigualable, ideal para quienes buscan una experiencia sofisticada.",
    features: 
      "Brandy Torres 15 se destaca por su sabor armonioso con notas de frutas secas, madera y vainilla. Ideal para degustarlo solo, en las rocas o en un cóctel especial.",
    seo: {
      keywords: [
        "Brandy Torres 15",
        "Brandy premium",
        "Brandy Torres 15 precio",
        "Brandy para regalo",
        "Compra brandy online",
        "Brandy cerca de mí",
        "Brandy de lujo",
        "Brandy Torres 15 online"
      ],
      title: "Brandy Torres 15 - De Lujo y Tradición",
      metaDescription: 
        "Compra Brandy Torres 15, una bebida de lujo de sabor suave y sofisticado. Ideal para ocasiones especiales. Descubre por qué es un favorito entre los conocedores.",
      locationTags: [
        "brandy en Ciudad de México",
        "brandy en Toluca",
        "brandy en Estado de México",
        "brandy cerca de mí"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Brandy Torres 15 (700 ml)"
      },
      {
        quantity: 1,
        item: "Etiqueta de garantía y autenticidad"
      },
      {
        quantity: 1,
        item: "Folleto informativo sobre la historia de Torres"
      }
    ],
    others: [
      {
        slug: "cognac-martell-vs",
        name: "Cognac Martell VS",
        image: {
          mobile: "/assets/brandies/martell-vs.webp",
          tablet: "/assets/brandies/martell-vs.webp",
          desktop: "/assets/brandies/martell-vs.webp"
        },
        seo: {
          keywords: [
            "Cognac Martell VS",
            "Cognac premium",
            "Martell VS precio",
            "Cognac para regalo",
            "Compra cognac online",
            "Cognac cerca de mí",
            "Cognac Martell VS online"
          ],
          title: "Cognac Martell VS - La Tradición de Francia",
          metaDescription:
            "Compra Cognac Martell VS, un cognac suave y elegante con notas de frutas secas y especias. Perfecto para disfrutar solo o en cócteles sofisticados."
        }
      },
      {
        slug: "brandy-torres-20",
        name: "Brandy Torres 20",
        image: {
          mobile: "/assets/brandies/brandy-torres-20-700ml.webp",
          tablet: "/assets/brandies/brandy-torres-20-700ml.webp",
          desktop: "/assets/brandies/brandy-torres-20-700ml.webp"
        },
        seo: {
          keywords: [
            "Brandy Torres 20",
            "Brandy premium",
            "Brandy Torres 20 precio",
            "Brandy para regalo",
            "Brandy Torres 20 online",
            "Compra brandy online",
            "Brandy Torres 20 cerca de mí"
          ],
          title: "Brandy Torres 20 - Exclusividad y Sabor",
          metaDescription:
            "Compra Brandy Torres 20, una bebida exclusiva con un sabor profundo y complejo. Ideal para ocasiones especiales y como regalo para los verdaderos conocedores."
        }
      },
      {
        slug: "cognac-martell-vsop",
        name: "Cognac Martell VSOP",
        image: {
          mobile: "/assets/brandies/martell-vsop.webp",
          tablet: "/assets/brandies/martell-vsop.webp",
          desktop: "/assets/brandies/martell-vsop.webp"
        },
        seo: {
          keywords: [
            "Cognac Martell VSOP",
            "Cognac premium",
            "Martell VSOP precio",
            "Cognac Martell",
            "Cognac para cócteles",
            "Cognac Martell VSOP online",
            "Cognac Martell VSOP para regalar"
          ],
          title: "Cognac Martell VSOP - Elegancia y Sofisticación",
          metaDescription:
            "Compra Cognac Martell VSOP, un cognac de lujo con una mezcla armoniosa de frutas y especias. Ideal para disfrutar en las mejores ocasiones."
        }
      }
    ]
  },
  {
    id: 15,
    slug: "vino-tinto-dolce-amore",
    name: "Vino Tinto Dolce Amore",
    image: {
      mobile: "/assets/vinos/dolce-amore.webp",
      tablet: "/assets/vinos/dolce-amore.webp",
      desktop: "/assets/vinos/dolce-amore.webp"
    },
    category: "vinos",
    categoryImage: {
      mobile: "/assets/vinos/dolce-amore.webp",
      tablet: "/assets/vinos/dolce-amore.webp",
      desktop: "/assets/vinos/dolce-amore.webp"
    },
    new: true,
    price: 450,
    description: "Disfruta de la suavidad y elegancia del Vino Tinto Dolce Amore, un vino premium que ofrece una rica mezcla de sabores afrutados y toques suaves de roble. Perfecto para acompañar cenas, celebraciones o disfrutar solo.",
    features: "El Vino Tinto Dolce Amore está elaborado con uvas seleccionadas, proporcionando un sabor profundo y armónico. Con una crianza en barricas de roble, ofrece notas de frutos rojos, ciruelas y un toque sutil de especias que lo hacen ideal para acompañar carnes rojas, quesos curados y pastas con salsa.",
    seo: {
      keywords: [
        "Vino Tinto Dolce Amore",
        "Vino tinto premium",
        "Vino tinto cerca de mí",
        "Vino para cena",
        "Vino tinto de calidad",
        "Vino Dolce Amore precio",
        "Vino para regalar",
        "Vino tinto para celebraciones"
      ],
      title: "Vino Tinto Dolce Amore - Vino Premium para los Amantes del Buen Sabor",
      metaDescription: "Compra el Vino Tinto Dolce Amore, un vino premium con una rica mezcla de sabores afrutados y toques suaves de roble. Perfecto para acompañar tus mejores momentos.",
      locationTags: [
        "vino tinto en Municipio",
        "vino tinto en Ciudad de México",
        "vino tinto en Toluca",
        "vino tinto cerca de mí",
        "vino tinto en Estado de México"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Vino Tinto Dolce Amore (750 ml)"
      },
      {
        quantity: 1,
        item: "Etiqueta de garantía y autenticidad"
      }
    ],
    others: [
      {
        slug: "vino-tinto-lambrusco",
        name: "Vino Tinto Lambrusco",
        image: {
          mobile: "/assets/vinos/lambrusco.webp",
          tablet: "/assets/vinos/lambrusco.webp",
          desktop: "/assets/vinos/lambrusco.webp"
        },
        seo: {
          keywords: [
            "Vino Tinto Lambrusco",
            "Vino tinto italiano",
            "Mejores vinos tinto",
            "Vino para cena italiana",
            "Vino Lambrusco precio",
            "Vino Lambrusco para regalar"
          ],
          title: "Vino Tinto Lambrusco - Vino Italiano de Calidad",
          metaDescription: "Compra el Vino Tinto Lambrusco, un vino italiano con un sabor fresco y afrutado. Ideal para acompañar platos italianos y compartir en ocasiones especiales."
        }
      },
      {
        slug: "vino-tinto-sangre-de-cristo",
        name: "Vino Tinto Sangre de Cristo",
        image: {
          mobile: "/assets/vinos/sangre-de-cristo.webp",
          tablet: "/assets/vinos/sangre-de-cristo.webp",
          desktop: "/assets/vinos/sangre-de-cristo.webp"
        },
        seo: {
          keywords: [
            "Vino Tinto Sangre de Cristo",
            "Vino tinto de México",
            "Vino para celebraciones",
            "Vino tinto premium",
            "Vino Sangre de Cristo precio",
            "Vino tinto para cena"
          ],
          title: "Vino Tinto Sangre de Cristo - Vino Mexicano Exquisito",
          metaDescription: "Disfruta del Vino Tinto Sangre de Cristo, un vino mexicano de alta calidad, ideal para acompañar carnes y celebraciones especiales."
        }
      },
      {
        slug: "vino-tinto-stella-rosa",
        name: "Vino Tinto Stella Rosa",
        image: {
          mobile: "/assets/vinos/stella-rosa.webp",
          tablet: "/assets/vinos/stella-rosa.webp",
          desktop: "/assets/vinos/stella-rosa.webp"
        },
        seo: {
          keywords: [
            "Vino Tinto Stella Rosa",
            "Vino tinto dulce",
            "Vino tinto de calidad",
            "Vino tinto para regalo",
            "Vino tinto premium",
            "Stella Rosa precio"
          ],
          title: "Vino Tinto Stella Rosa - Vino Dulce y Premium",
          metaDescription: "Compra el Vino Tinto Stella Rosa, un vino dulce y afrutado, perfecto para regalar o disfrutar en ocasiones especiales."
        }
      }
    ]
  },
  {
    id: 16,
    slug: "vino-tinto-sweet-red-reservado",
    name: "Vino tinto Sweet Red Reservado",
    image: {
      mobile: "/assets/vinos/sweet-red-reservado.webp",
      tablet: "/assets/vinos/sweet-red-reservado.webp",
      desktop: "/assets/vinos/sweet-red-reservado.webp"
    },
    category: "vinos",
    categoryImage: {
      mobile: "/assets/vinos/sweet-red-reservado.webp",
      tablet: "/assets/vinos/sweet-red-reservado.webp",
      desktop: "/assets/vinos/sweet-red-reservado.webp"
    },
    new: true,
    price: 399,
    description:
      "Deléitate con el exquisito sabor del Vino tinto Sweet Red Reservado, un vino de cuerpo medio con notas de frutos rojos maduros y un toque dulce que lo hace perfecto para cualquier ocasión especial. Disfrútalo acompañado de tus platillos favoritos o simplemente solo.",
    features:
      "El Vino tinto Sweet Red Reservado combina la suavidad y dulzura de los mejores vinos tintos. Producido con uvas seleccionadas, ofrece un perfil de sabor equilibrado con notas de cereza, frambuesa y un delicado aroma floral.\n\nIdeal para cenas románticas, reuniones familiares o como regalo especial. Este vino destaca por su presentación elegante y versatilidad, siendo la opción perfecta para los amantes del vino tinto.",
    seo: {
      keywords: [
        "Vino tinto Sweet Red Reservado",
        "Vinos dulces",
        "Vino tinto cerca de mí",
        "Compra vino online",
        "Vino tinto afrutado",
        "Vino dulce recomendado",
        "Mejores vinos tintos"
      ],
      title: "Sweet Red Reservado - Vino Tinto Dulce y Elegante",
      metaDescription:
        "Compra el Vino tinto Sweet Red Reservado, una opción dulce y afrutada perfecta para toda ocasión. Descubre su sabor único con notas de frutos rojos y un toque floral.",
      locationTags: [
        "vino tinto en Municipio",
        "vino tinto en Ciudad de México",
        "vino tinto en Guadalajara",
        "vino tinto cerca de mí",
        "vino en Estado de México"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Vino tinto Sweet Red Reservado (750 ml)"
      },
      {
        quantity: 1,
        item: "Etiqueta de calidad y autenticidad"
      },
      {
        quantity: 1,
        item: "Folleto con información sobre maridajes ideales"
      }
    ],
    others: [
      {
        slug: "vino-tinto-dolce-amore",
        name: "Vino tinto Dolce Amore",
        image: {
          mobile: "/assets/vinos/dolce-amore.webp",
          tablet: "/assets/vinos/dolce-amore.webp",
          desktop: "/assets/vinos/dolce-amore.webp"
        },
        seo: {
          keywords: [
            "Vino tinto Dolce Amore",
            "Vinos dulces",
            "Vino tinto recomendado",
            "Compra vino online",
            "Vino Dolce Amore precio"
          ],
          title: "Dolce Amore - Vino Tinto Dulce y Aromático",
          metaDescription:
            "Prueba el Vino tinto Dolce Amore, un vino suave y dulce con notas de frutas rojas y un aroma embriagador. Perfecto para cualquier ocasión."
        }
      },
      {
        slug: "vino-tinto-4-soles-afrutado",
        name: "Vino tinto 4 Soles Afrutado",
        image: {
          mobile: "/assets/vinos/4-soles-afrutado.webp",
          tablet: "/assets/vinos/4-soles-afrutado.webp",
          desktop: "/assets/vinos/4-soles-afrutado.webp"
        },
        seo: {
          keywords: [
            "Vino tinto 4 Soles Afrutado",
            "Vino tinto mexicano",
            "Vino afrutado",
            "Compra vino 4 Soles",
            "Mejores vinos mexicanos"
          ],
          title: "4 Soles Afrutado - Vino Tinto Mexicano",
          metaDescription:
            "Disfruta el Vino tinto 4 Soles Afrutado, una opción ligera y refrescante ideal para maridar con platillos mexicanos y mediterráneos."
        }
      },
      {
        slug: "vino-tinto-lambrusco",
        name: "Vino tinto Lambrusco",
        image: {
          mobile: "/assets/vinos/lambrusco.webp",
          tablet: "/assets/vinos/lambrusco.webp",
          desktop: "/assets/vinos/lambrusco.webp"
        },
        seo: {
          keywords: [
            "Vino tinto Lambrusco",
            "Vino italiano",
            "Lambrusco precio",
            "Compra vino online",
            "Vino tinto Lambrusco dulce"
          ],
          title: "Lambrusco - Vino Tinto Espumoso Italiano",
          metaDescription:
            "Explora el Vino tinto Lambrusco, un vino espumoso italiano con un sabor dulce y refrescante, ideal para celebraciones y momentos especiales."
        }
      }
    ]
  },
  {
    id: 17,
    slug: "vino-tinto-4-soles-afrutado",
    name: "Vino Tinto 4 Soles Afrutado",
    image: {
      mobile: "/assets/vinos/4-soles-afrutado.webp",
      tablet: "/assets/vinos/4-soles-afrutado.webp",
      desktop: "/assets/vinos/4-soles-afrutado.webp",
    },
    category: "vinos",
    categoryImage: {
      mobile: "/assets/vinos/4-soles-afrutado.webp",
      tablet: "/assets/vinos/4-soles-afrutado.webp",
      desktop: "/assets/vinos/4-soles-afrutado.webp",
    },
    new: true,
    price: 199,
    description:
      "Experimenta el sabor fresco y afrutado del Vino Tinto 4 Soles Afrutado. Este vino es ideal para quienes buscan un balance perfecto entre dulzura y cuerpo, con notas intensas de frutas rojas y un final suave. Perfecto para acompañar momentos especiales o disfrutarlo solo.",
    features:
      "El Vino Tinto 4 Soles Afrutado ofrece un perfil aromático encantador con matices de cerezas, fresas y un toque de frambuesa. Con una textura sedosa y un cuerpo ligero, este vino es ideal para maridar con postres, quesos suaves o simplemente disfrutar en una tarde relajada.\n\nDisponible en botellas de 750 ml, es el regalo perfecto para los amantes del vino y una excelente opción para reuniones informales o celebraciones.",
    seo: {
      keywords: [
        "Vino Tinto 4 Soles Afrutado",
        "Vino tinto afrutado",
        "Compra vino online",
        "Vino dulce",
        "Vino tinto cerca de mí",
        "4 Soles precio",
        "Vino afrutado mexicano",
        "Mejores vinos para regalar",
      ],
      title: "Vino Tinto 4 Soles Afrutado - Sabor Fresco y Dulzura",
      metaDescription:
        "Descubre el Vino Tinto 4 Soles Afrutado, un vino fresco y dulce con notas de frutas rojas. Perfecto para compartir en ocasiones especiales o disfrutar solo.",
      locationTags: [
        "vino en Ciudad de México",
        "vino en Toluca",
        "vino cerca de mí",
        "vino en Estado de México",
      ],
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Vino Tinto 4 Soles Afrutado (750 ml)",
      },
      {
        quantity: 1,
        item: "Etiqueta de garantía de calidad",
      },
      {
        quantity: 1,
        item: "Folleto con maridajes recomendados",
      },
    ],
    others: [
      {
        slug: "vino-tinto-sangre-de-cristo",
        name: "Vino Tinto Sangre de Cristo",
        image: {
          mobile: "/assets/vinos/sangre-de-cristo.webp",
          tablet: "/assets/vinos/sangre-de-cristo.webp",
          desktop: "/assets/vinos/sangre-de-cristo.webp",
        },
        seo: {
          keywords: [
            "Vino Tinto Sangre de Cristo",
            "Vino tinto mexicano",
            "Compra vino online",
            "Mejores vinos",
            "Vino dulce afrutado",
            "Vino Sangre de Cristo precio",
          ],
          title: "Vino Tinto Sangre de Cristo - Tradición y Sabor Único",
          metaDescription:
            "Disfruta del Vino Tinto Sangre de Cristo, un clásico mexicano con notas intensas y afrutadas. Ideal para cenas y celebraciones especiales.",
        },
      },
      {
        slug: "vino-tinto-sweet-red-reservado",
        name: "Vino Tinto Sweet Red Reservado",
        image: {
          mobile: "/assets/vinos/sweet-red-reservado.webp",
          tablet: "/assets/vinos/sweet-red-reservado.webp",
          desktop: "/assets/vinos/sweet-red-reservado.webp",
        },
        seo: {
          keywords: [
            "Vino Tinto Sweet Red Reservado",
            "Vino dulce mexicano",
            "Vino premium",
            "Vino tinto cerca de mí",
            "Sweet Red Reservado precio",
            "Mejores vinos para regalar",
          ],
          title: "Sweet Red Reservado - Vino Dulce Premium",
          metaDescription:
            "Prueba el Vino Tinto Sweet Red Reservado, con un sabor exquisito y dulce que lo convierte en la elección ideal para postres o celebraciones especiales.",
        },
      },
      {
        slug: "vino-tinto-dolce-amore",
        name: "Vino Tinto Dolce Amore",
        image: {
          mobile: "/assets/vinos/dolce-amore.webp",
          tablet: "/assets/vinos/dolce-amore.webp",
          desktop: "/assets/vinos/dolce-amore.webp",
        },
        seo: {
          keywords: [
            "Vino Tinto Dolce Amore",
            "Vino afrutado dulce",
            "Vino tinto ligero",
            "Compra vino Dolce Amore",
            "Mejores vinos afrutados",
            "Dolce Amore precio",
          ],
          title: "Dolce Amore - Vino Tinto Afrutado",
          metaDescription:
            "Disfruta de Dolce Amore, un vino tinto afrutado y dulce con un perfil ligero. Perfecto para cualquier ocasión o como acompañamiento de postres.",
        },
      },
    ],
  },
  {
    id: 18,
    slug: "baileys-original",
    name: "Baileys Original",
    image: {
      mobile: "/assets/licores/baileys-original.webp",
      tablet: "/assets/licores/baileys-original.webp",
      desktop: "/assets/licores/baileys-original.webp",
    },
    category: "licores",
    categoryImage: {
      mobile:
        "/assets/licores/baileys-original.webp",
      tablet:
        "/assets/licores/baileys-original.webp",
      desktop:
        "/assets/licores/baileys-original.webp",
    },
    new: true,
    price: 499,
    description:
      "Disfruta del sabor único y cremoso de Baileys Original, el licor icónico que combina crema de leche irlandesa con whisky cuidadosamente seleccionado. Perfecto para tomar solo, con hielo o como base para deliciosos postres y cócteles.",
    features:
      "Baileys Original es un licor premium que destaca por su cremosidad y sabor equilibrado. Hecho con ingredientes naturales y crema de leche irlandesa, es ideal para ocasiones especiales, postres y cócteles. Su presentación de 750 ml lo convierte en un regalo perfecto o un acompañante ideal para celebraciones.",
    seo: {
      keywords: [
        "Baileys Original",
        "Licor Baileys",
        "Mejores licores",
        "Licor cerca de mí",
        "Baileys precio",
        "Compra licor online",
        "Licor para cócteles",
        "Baileys recetas de postres"
      ],
      title: "Baileys Original - El Licor Cremoso Clásico",
      metaDescription:
        "Compra Baileys Original, el licor cremoso que combina crema irlandesa y whisky. Ideal para disfrutar solo, en cócteles o como complemento en postres.",
      locationTags: [
        "licor en Municipio",
        "licor en Ciudad de México",
        "licor en Toluca",
        "licor cerca de mí",
        "licor en Estado de México"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Baileys Original (750 ml)",
      },
      {
        quantity: 1,
        item: "Etiqueta de garantía y autenticidad",
      }
    ],
    others: [
      {
        slug: "flamingo-curacao",
        name: "Licor Flamingo Curacao",
        image: {
          mobile: "/assets/licores/flamingo-curacao.webp",
          tablet: "/assets/licores/flamingo-curacao.webp",
          desktop: "/assets/licores/flamingo-curacao.webp",
        },
        seo: {
          keywords: [
            "Licor Flamingo Curacao",
            "Mejores licores",
            "Licor para cócteles",
            "Flamingo Curacao precio",
            "Compra licor online",
            "Licor para regalo"
          ],
          title: "Flamingo Curacao - Licor Perfecto para Cócteles",
          metaDescription:
            "Flamingo Curacao es un licor vibrante y dulce ideal para crear los mejores cócteles. Descubre su sabor único y dale un toque especial a tus bebidas.",
        }
      },
      {
        slug: "licor-rancho-escondido",
        name: "Licor De Agave Rancho Escondido",
        image: {
          mobile: "/assets/licores/rancho-escondido.webp",
          tablet: "/assets/licores/rancho-escondido.webp",
          desktop: "/assets/licores/rancho-escondido.webp",
        },
        seo: {
          keywords: [
            "Licor De Agave Rancho Escondido",
            "Licor de agave premium",
            "Rancho Escondido precio",
            "Compra licor de agave online",
            "Mejores licores",
            "Rancho Escondido licor mexicano"
          ],
          title: "Rancho Escondido - Licor de Agave Mexicano",
          metaDescription:
            "Descubre el Licor De Agave Rancho Escondido, una bebida con el auténtico sabor mexicano. Perfecto para celebraciones y cócteles especiales.",
        }
      },
      {
        slug: "licor-jagermeister",
        name: "Licor Jagermeister",
        image: {
          mobile: "/assets/licores/jagermeister.webp",
          tablet: "/assets/licores/jagermeister.webp",
          desktop: "/assets/licores/jagermeister.webp",
        },
        seo: {
          keywords: [
            "Licor Jagermeister",
            "Mejores licores",
            "Jagermeister precio",
            "Compra Jagermeister online",
            "Licor para shots",
            "Licor herbal premium"
          ],
          title: "Jagermeister - Licor Herbal Icónico",
          metaDescription:
            "Jagermeister es un licor herbal único, ideal para shots o cócteles. Descubre su sabor inconfundible y disfruta de su tradición alemana.",
        }
      }
    ],
  },
  {
    id: 19,
    slug: "licor-jagermeister",
    name: "Licor Jägermeister",
    image: {
      mobile: "/assets/licores/jagermeister.webp",
      tablet: "/assets/licores/jagermeister.webp",
      desktop: "/assets/licores/jagermeister.webp",
    },
    category: "licores",
    categoryImage: {
      mobile:
        "/assets/licores/jagermeister.webp",
      tablet:
        "/assets/licores/jagermeister.webp",
      desktop:
        "/assets/licores/jagermeister.webp",
    },
    new: true,
    price: 499,
    description:
      "El Licor Jägermeister es una bebida alemana icónica, elaborada con 56 ingredientes naturales cuidadosamente seleccionados, entre hierbas, flores, raíces y frutas. Su sabor único y balanceado lo convierte en el complemento perfecto para cualquier ocasión, ya sea en un shot bien frío o como base para cócteles creativos.",
    features:
      "Jägermeister es un licor premium conocido por su proceso de elaboración artesanal, que incluye maceración en frío y un envejecimiento de varios meses en barricas de roble. Su sabor intenso y versátil es ideal para disfrutarlo en celebraciones, reuniones o simplemente relajándote.\n\nDisponible en presentaciones de 700 ml, Jägermeister es un clásico que no puede faltar en tu bar personal. Su versatilidad lo hace perfecto para combinar con bebidas energéticas, jugos o incluso como ingrediente en la cocina.",
    seo: {
      keywords: [
        "Licor Jägermeister",
        "Jägermeister precio",
        "Compra Jägermeister online",
        "Licor alemán",
        "Licor premium",
        "Jägermeister cerca de mí",
        "Mejores licores",
        "Bebidas premium",
        "Shot de Jägermeister",
        "Cócteles con Jägermeister"
      ],
      title: "Jägermeister - Licor Alemán Premium",
      metaDescription:
        "Compra Jägermeister, el licor alemán icónico con 56 ingredientes naturales. Perfecto para disfrutar en shots fríos o en cócteles innovadores. ¡Descúbrelo ahora!",
      locationTags: [
        "licor en Municipio",
        "licor en Ciudad de México",
        "licor en Toluca",
        "licor cerca de mí",
        "licor en Estado de México"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Licor Jägermeister (700 ml)",
      },
      {
        quantity: 1,
        item: "Folleto de recetas con Jägermeister",
      },
      {
        quantity: 1,
        item: "Etiqueta de autenticidad",
      },
    ],
    others: [
      {
        slug: "baileys-original",
        name: "Baileys Original",
        image: {
          mobile: "/assets/licores/baileys-original.webp",
          tablet: "/assets/licores/baileys-original.webp",
          desktop: "/assets/licores/baileys-original.webp",
        },
        seo: {
          keywords: [
            "Baileys Original",
            "Licor de crema",
            "Compra Baileys online",
            "Baileys precio",
            "Licor Baileys para postres",
            "Cócteles con Baileys",
            "Mejores licores",
            "Baileys cerca de mí"
          ],
          title: "Baileys Original - Licor de Crema Premium",
          metaDescription:
            "Disfruta del clásico Baileys Original, el licor de crema perfecto para postres y cócteles. Compra en línea y eleva tus momentos especiales.",
        }
      },
      {
        slug: "licor-rancho-escondido-tamarindo",
        name: "Licor de Agave Rancho Escondido Tamarindo",
        image: {
          mobile: "/assets/licores/rancho-escondido-tamarindo.webp",
          tablet: "/assets/licores/rancho-escondido-tamarindo.webp",
          desktop: "/assets/licores/rancho-escondido-tamarindo.webp",
        },
        seo: {
          keywords: [
            "Rancho Escondido Tamarindo",
            "Licor de agave tamarindo",
            "Compra Rancho Escondido online",
            "Licor mexicano",
            "Cócteles con tamarindo",
            "Mejores licores",
            "Rancho Escondido precio"
          ],
          title: "Rancho Escondido Tamarindo - Licor Mexicano con Sabor Único",
          metaDescription:
            "Descubre Rancho Escondido Tamarindo, el licor mexicano con un toque dulce y ácido perfecto para shots y cócteles. ¡Pruébalo ahora!",
        }
      },
      {
        slug: "licor-43",
        name: "Licor 43",
        image: {
          mobile: "/assets/licores/licor-43.webp",
          tablet: "/assets/licores/licor-43.webp",
          desktop: "/assets/licores/licor-43.webp",
        },
        seo: {
          keywords: [
            "Licor 43",
            "Licor premium",
            "Mejores licores",
            "Compra Licor 43 online",
            "Licor 43 precio",
            "Cócteles con Licor 43",
            "Licor español",
            "Bebidas con Licor 43"
          ],
          title: "Licor 43 - El Licor Premium de España",
          metaDescription:
            "Disfruta del sabor único del Licor 43, una mezcla de cítricos y vainilla que eleva tus cócteles. Compra en línea y sorprende a tus invitados.",
        }
      }
    ],
  },
  {
    id: 2,
    slug: "licor-agave-rancho-escondido-tamarindo",
    name: "Licor Rancho Escondido Tamarindo",
    image: {
      mobile: "/assets/licores/rancho-escondido-tamarindo.webp",
      tablet: "/assets/licores/rancho-escondido-tamarindo.webp",
      desktop: "/assets/licores/rancho-escondido-tamarindo.webp",
    },
    category: "licores",
    categoryImage: {
      mobile:
        "/assets/licores/rancho-escondido-tamarindo.webp",
      tablet:
        "/assets/licores/rancho-escondido-tamarindo.webp",
      desktop:
        "/assets/licores/rancho-escondido-tamarindo.webp",
    },
    new: true,
    price: 199,
    description:
      "Disfruta del Licor De Agave Rancho Escondido Tamarindo, una deliciosa mezcla de agave con el toque ácido y dulce del tamarindo. Perfecto para compartir en reuniones o para disfrutar en cocteles refrescantes.",
    features:
      "El Licor De Agave Rancho Escondido Tamarindo ofrece un sabor auténtico y único, con un equilibrio ideal entre dulzura y acidez. Ideal para beber solo, en las rocas o como base para cocteles. Este licor es una excelente opción para quienes buscan una experiencia refrescante y diferente.\n\nDisponible en presentación de 750 ml, es perfecto para cualquier ocasión, desde reuniones casuales hasta celebraciones especiales.",
    seo: {
      keywords: [
        "Licor De Agave Rancho Escondido Tamarindo",
        "Licor de tamarindo",
        "Licores premium",
        "Licores cerca de mí",
        "Rancho Escondido Tamarindo precio",
        "Compra licor online",
        "Licor para cocteles",
        "Licor de agave con tamarindo",
      ],
      title: "Rancho Escondido Tamarindo - Licor De Agave",
      metaDescription:
        "Compra Rancho Escondido Tamarindo, un licor de agave con el toque ácido-dulce del tamarindo. Ideal para cocteles o disfrutarlo solo. Pídelo ahora.",
      locationTags: [
        "licor en Municipio",
        "licor en Ciudad de México",
        "licor en Toluca",
        "licor cerca de mí",
        "licor en Estado de México"
      ]
    },
    includes: [
      {
        quantity: 1,
        item: "Botella de Licor Rancho Escondido Tamarindo (750 ml)",
      },
    ],
    others: [
      {
        slug: "licor-43",
        name: "Licor 43",
        image: {
          mobile: "/assets/licores/licor-43.webp",
          tablet: "/assets/licores/licor-43.webp",
          desktop: "/assets/licores/licor-43.webp",
        },
        seo: {
          keywords: [
            "Licor 43",
            "Licor premium",
            "Licor cerca de mí",
            "Licor 43 precio",
            "Compra licor online",
            "Licor para cocteles",
          ],
          title: "Licor 43 - Licor Premium de Tradición",
          metaDescription:
            "Descubre el Licor 43, un licor premium con sabores únicos de vainilla y especias, perfecto para disfrutar solo o en cocteles.",
        }
      },
      {
        slug: "licor-agave-rancho-escondido-mango",
        name: "Rancho Escondido Mango",
        image: {
          mobile: "/assets/licores/rancho-escondido-mago.webp",
          tablet: "/assets/licores/rancho-escondido-mago.webp",
          desktop: "/assets/licores/rancho-escondido-mago.webp",
        },
        seo: {
          keywords: [
            "Licor De Agave Rancho Escondido Mango",
            "Licor de mango",
            "Rancho Escondido Mango precio",
            "Compra licor online",
            "Licores frutales",
          ],
          title: "Rancho Escondido Mango - Licor De Agave",
          metaDescription:
            "Compra Rancho Escondido Mango, un licor de agave con el sabor tropical del mango. Perfecto para cocteles refrescantes o disfrutarlo solo.",
        }
      },
      {
        slug: "rompope-coronado",
        name: "Rompope Coronado",
        image: {
          mobile: "/assets/licores/rompope-coronado.webp",
          tablet: "/assets/licores/rompope-coronado.webp",
          desktop: "/assets/licores/rompope-coronado.webp",
        },
        seo: {
          keywords: [
            "Rompope Coronado",
            "Rompope tradicional",
            "Rompope premium",
            "Rompope cerca de mí",
            "Rompope Coronado precio",
            "Compra rompope online",
          ],
          title: "Rompope Coronado - El Sabor Tradicional",
          metaDescription:
            "Disfruta del Rompope Coronado, el clásico licor cremoso con el auténtico sabor mexicano. Perfecto para ocasiones especiales o como postre.",
        }
      },
    ],
},
];
